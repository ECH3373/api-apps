...
const index = async (req, res) => {
  const { data, meta } = await services.crud.index({ model: 'app', query: req.query, search: ['name'] });
  return services.response.send({ res, data, meta, message: 'the list of apps has been successfully retrieved' });
};

const show = async (req, res) => {
  const data = await services.crud.show({ model: 'app', target: req.params.id, keys: ['id'] });
